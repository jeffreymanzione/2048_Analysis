...
public class RandomLookAheadPlayer2048 extends Player2048 {
	...
	public RandomLookAheadPlayer2048(int looks, Function<MoveSequence, Integer> fun) {
		this.looks = looks;
		this.fun = fun;
	}
	...
	private List<MoveSequence> choose(List<MoveSequence> seqs, int level) {
		if (level > 0) {
			...
			seqs.forEach(seq -> randoms(Game2048.ifShifts(seq.last.board)).stream().forEach(
					mov -> news.add(seq.add(mov))));
	...
	private Collection<Move> randoms(List<Move> ifShifts) {
		List<Move> allPossible = new ArrayList<>();

		for (Move move : ifShifts) {
			for (int i = 0; i < 4; i++) {
				for (int j = 0; j < 4; j++) {
					...
					if (move.board[i][j] == 0) {
						for (int k = 1; k <= 2; k++) {
							int[][] board = Game2048.copyBoard(move.board);
							board[i][j] = k;
							temp.add(new Move(board, move.score, move.numMerges, move.dir, move.maxBlock));
						}
						count++;
					}

					if (count > 1) {
						allPossible.addAll(temp);
					}
				}
			}
		}
